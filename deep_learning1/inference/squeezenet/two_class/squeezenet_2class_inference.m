function squeezenet_2class_inference(inputFolder, outputFolder, trainedNetworkFile, labelFile)
% SQUEEZENET_2CLASS_INFERENCE
%
% Perform inference on Hartmann-Shack (HS) images using a pre-trained
% SqueezeNet-based binary classification model.
%
...
%
% Usage:
%   squeezenet_2class_inference(inputFolder, outputFolder, trainedNetworkFile)
%   squeezenet_2class_inference(inputFolder, outputFolder, trainedNetworkFile, labelFile)
%
% Inputs:
...
%   - Input images are resized to match the SqueezeNet input size
%     (typically 227x227x3, depending on the trained network)
%   - ImageNet normalization is applied, matching the training pipeline
    ...
    for i = 1:numImages
        ...
        % Resize image to match the input size required by SqueezeNet (typically 227x227)
...
%
% Notes:
%   - Resize is not performed here; it is applied later in the main loop
%   - This preprocessing matches the training pipeline used for transfer learning
...
% GETGROUNDTRUTHLABEL Retrieve ground-truth label.
%
% Supports both numeric labels (0/1) and text labels ("valid"/"invalid").
