function train_shufflenet_kfold(dataFolder, foldFolder, baseNetworkFile, outputFolder, numFolds)
% TRAIN_SHUFFLENET_KFOLD
%
% Train and evaluate a ShuffleNet-based using k-fold
% cross-validation.
%
% Usage:
%   train_shufflenet_kfold(dataFolder, foldFolder, baseNetworkFile, outputFolder)
%   train_shufflenet_kfold(dataFolder, foldFolder, baseNetworkFile, outputFolder, numFolds)
%
% Inputs:
...
%   numFolds         - Number of folds (default: 5)
    ...
    for foldIdx = 1:numFolds
        ...
        modelFile = fullfile(modelsFolder, sprintf('shufflenet_fold_%d.mat', foldIdx));
        save(modelFile, 'net');
        ...
        title(sprintf('ShuffleNet - Fold %d', foldIdx));
    ...
    summaryFile = fullfile(outputFolder, 'shufflenet_kfold_results.xlsx');
...
% READANDCONVERT Read image and convert it to ShuffleNet-compatible format.
%
% This function:
%   1. Reads the image
