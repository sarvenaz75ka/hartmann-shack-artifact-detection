...
% Define the root folder containing the image data
folderName = 'C:\Ainaimage\16\2 classification\Input';
...
inputSize = ShuffleNet_2class.Layers(1).InputSize;
...
for i = 1:numFolds
    ...
    options = trainingOptions("adam", ... % Use the Adam optimizer
        MaxEpochs=7, ... % Maximum number of epochs
    ...
    net = trainnet(augimdsTrain, ShuffleNet_2class, "crossentropy", options);
...
% % testFolder = 'C:\Ainaimage\16\2 classification\Data_5Folds';
% 
...
% numImagesToTest = numel(filePaths);
% 
...
% net_cpu = gather(net); 
% inputLayer = net_cpu.Layers(1); % فرض بر این است که لایه اول، لایه ورودی تصویر است
